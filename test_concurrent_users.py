...
def crunch_numbers(max_users, resultfilename):
	results = []
	for num_users in xrange(1, max_users+1):
		result_dict = crunch_numbers_for_users(num_users)
...
def crunch_numbers_for_users(num_users):
	...
	for user in range(num_users):
		path = "./resultsqueued_1k_idx_1_users_%s/%s/" %(num_users, user)
...
def run_benchmark(max_users, resultfilename):
	for num_users in xrange(1, max_users+1):
		print ""
		...
		os.rename("./resultsqueued_1k_idx_1", "resultsqueued_1k_idx_1_users_"+str(num_users))
	crunch_numbers(max_users, resultfilename)
...
def start_server(settingsfile):
	...
	exp_env["LD_LIBRARY_PATH"] = "." + bin_dir
	...
	proc = subprocess.Popen(["."+bin_dir+"hyrise_server"], env=exp_env) #stdout=subprocess.PIPE, stderr=subprocess.PIPE
	os.chdir(cwd)
	time.sleep(1)
	return proc
...
for s in settingsfiles:
	...
	p = start_server(s)
	try:
		run_benchmark(max_users=5, resultfilename="result_"+s+".csv")
	finally:
		kill_server(p)
